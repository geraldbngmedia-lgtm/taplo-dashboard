...
import taploMark from "@/assets/taplo-logo-mark.png.asset.json";
...
export function DashboardAssistant() {
  ...
  const composer = (
    <div className="w-full max-w-[720px]">
      <PromptInput
        ...
        className="dashboard-composer rounded-[28px] border-[var(--glass-border)] bg-[var(--dashboard-glass)] shadow-[var(--shadow-composer)] backdrop-blur-[24px]"
      >
        <PromptInputTextarea
          ref={textareaRef}
          value={input}
          ...
          rows={1}
          className="min-h-[58px] resize-none px-5 pt-4 text-[15px] leading-6 text-[var(--ink)] placeholder:text-[var(--ink-faint)]"
        />
  ...
  return (
    <section className="dashboard-chat-canvas relative flex h-full min-h-0 flex-col overflow-hidden">
      <div className="dashboard-toolbar-wrap absolute right-5 top-5 z-20 flex items-center gap-2 sm:right-8 sm:top-7">
        <Button
          type="button"
          variant="ghost"
          size="icon-sm"
        ...
        <Popover>
          ...
          <PopoverContent
            align="end"
            sideOffset={8}
            className="w-[310px] rounded-[16px] border-[var(--glass-border)] bg-[var(--dashboard-glass-strong)] p-2 shadow-[var(--shadow-glass-overlay)] backdrop-blur-2xl"
          >
      ...
      {isEmpty ? (
        <div className="dashboard-empty-state dashboard-panel-aware flex min-h-0 flex-1 flex-col items-center justify-center px-5 pb-20 pt-24 sm:px-10 max-[1000px]:justify-start max-[1000px]:pt-20">
          <div className="w-full max-w-[680px] text-center">
            <h1 className="font-[Plus_Jakarta_Sans] text-[clamp(32px,4vw,48px)] font-medium leading-[1.12] text-[var(--ink)]">
          ...
          <div className="mt-8 grid w-full max-w-[680px] gap-3 sm:grid-cols-3 max-[1000px]:mt-6 max-[1000px]:gap-2">
            {suggestions.map(({ icon: Icon, label }) => (
              <Button
                ...
                className="dashboard-suggestion h-auto min-h-[76px] justify-start whitespace-normal rounded-[16px] border-[var(--glass-border)] bg-[var(--dashboard-glass-soft)] p-4 text-left text-[13px] font-medium leading-5 text-[var(--ink)] shadow-[var(--shadow-glass-control)] backdrop-blur-xl max-[1000px]:min-h-[66px] max-[1000px]:p-3 max-[1000px]:text-[11px] max-[1000px]:leading-4"
              >
                <span className="flex flex-col items-start gap-3 max-[1000px]:gap-2">
          ...
          <div className="mt-7 flex w-full max-w-[680px] justify-center max-[1000px]:mt-5">
            {composer}
          </div>
        </div>
      ) : (
        <>
          <div className="dashboard-panel-aware flex h-16 shrink-0 items-center justify-center border-b border-[var(--glass-border-subtle)] px-20">
            <div className="min-w-0 text-center">
              <p className="truncate text-[13px] font-semibold text-[var(--ink)]">
                {activeThread?.title ?? "Ask Taplo"}
              </p>
              <p className="mt-0.5 text-[10px] text-[var(--ink-faint)]">
                Interview-grounded answer
              </p>
            </div>
          </div>

          <Conversation className="min-h-0">
            <ConversationContent className="dashboard-conversation-column w-full gap-9 px-5 pb-44 pt-10 sm:px-8">
                ...
                return (
                  ...
                  >
                    <MessageContent
                      className={
                        message.role === "user"
                          ? "max-w-[82%] rounded-[18px] border border-[var(--glass-border-subtle)] bg-[var(--chat-user-soft)] px-4 py-3 text-[14px] leading-6 text-[var(--ink)] shadow-[var(--shadow-message)] backdrop-blur-lg"
          ...
          <div className="dashboard-composer-dock dashboard-panel-aware absolute inset-x-0 bottom-0 z-10 flex justify-center bg-[var(--composer-dock)] px-5 pb-5 pt-12 sm:px-8 sm:pb-7">
